...
boxPltSalary$out
```
When we run the code "_boxPltSalary$out_" we get the outliers. We can see that there are **13 outliers** in the dataset.

## d. Make a reasonable train-test split of the data. Apply a cross-validation strategy with 5-fold and present your tuned parameter values.
...
metrics
```
From the above dataframe _metrics_ we can observe that *KNN* model has the

1. Lowest r2 value
2. Highest rmse value
3. Highest mae value
